...
def main():
    ...
    if "--user_harvest" in sys.argv:
        twitter_ops.harvest(env.run_folder, credentials, auth, api,
                            mongodb_config.client, mongodb_config.db, mongodb_config.collection) #why does this work if no credentials?!
    # if user wants the friend list, make it
    if "--get_following" in sys.argv:
        twitter_ops.get_following(env.run_folder, credentials, auth, api, mongodb_config.following_collection)
