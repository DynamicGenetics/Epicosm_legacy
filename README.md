...
`--liwc` Apply LIWC (Pennebaker et al 2015) analysis and append values to the local database. You must have a LIWC dictionary in therun folder, named "LIWC.dic". LIWC has around 70 categories (including posemo and negemo), but many of these will return no value because tweets are too short to provide information. Empty categories are not appended to the database. Note: Some versions of the LIWC dictionary contain lines that epicosm_nlp will not understand. You can remove these lines with the script in the `src` directory, `clean_LIWC.sh`. This will (very) slightly change your output scores, please assess this for yourself.
